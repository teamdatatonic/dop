...
from dop.component.transformation.runner.bigquery.adapter.model import TableOptionsConfig, PartitionConfig, \
    UDFArgument, \
    StoredProcedureArgument
from dop.component.transformation.runner.bigquery.adapter.relation import BigQueryRelation as Relation, RelationHelper
...
def execute_job_with_error_logging(job):
    if job.dry_run:
        logging.info(f'Total GB it will process: {job.total_bytes_processed / 1024 / 1024 / 1024}')
        return
...
def get_bq_client(project_id, location, credentials):
    return bigquery.client.Client(project=project_id, location=location, credentials=credentials)
...
def get_query_runner(options: Optional[Dict[str, Any]]):
    ...
    if env.env_config.is_sandbox_environment:
        credentials = auth.ServiceAccountImpersonationCredentialManager(
            source_sa_name=env.DOP_DOCKER_USER,
            project_id=project_id
        ).get_target_credentials()
        logging.info(f"Using service account impersonation, and service account `{env.DOP_DOCKER_USER}` is now active")

    client = get_bq_client(project_id=project_id, location=location, credentials=credentials)
...
class QueryRunner:
    ...
    def replace_or_upsert(self, query: str, relation: Relation, options: Optional[Dict[str, Any]] = None):
        """
        ...
        def query_cleaned(q):
            if q.strip()[:-1] == ';':
                q = q.strip()[:-1]
                if q.strip()[:-1] == ';':
                    raise RuntimeError(f'Query {query} should not be followed by `;` at the very end')
        ...
        query_job = self._client.query(query=rendered_create_or_replace, job_config=job_config)
    ...
    def recreate_view(self, query, relation: Relation):
        ...
        try:
            ...
            logging.info('View {} not found, ignore the delete operation'.format(relation))
            pass
    ...
    def create_schema(self, project_id, dataset_id, exists_ok=True):
        ...
        logging.info('New Dataset {} already exists or has been created'.format(full_dataset_id))
    ...
    def render_udf_query(arguments: List[Dict], query, relation):
        arguments_models = [UDFArgument(name=argument['name'], type=argument['type']) for argument in arguments]
        parsed_arguments = ','.join([f'{argument.name} {argument.type}' for argument in arguments_models])
        rendered_query = f"""
...
"""
    ...
    def render_stored_procedure_query(arguments: List[Dict], query, relation):
        arguments_models = [StoredProcedureArgument(name=argument['name'], type=argument['type']) for argument in
                            arguments]
        parsed_arguments = ','.join([f'{argument.name} {argument.type}' for argument in arguments_models])
        rendered_query = f"""
...
"""
    ...
    def assertion(self, query):
        ...
        try:
            ...
            logging.info("\n\n#### Assertion Report ####\n\n" + yaml_parser.dict_to_yaml(
                results['assertion_results']) + "\n\n#### Assertion Report ####\n\n")

            if results['has_failure']:
                raise AssertionError('Assertion failed, check the "ASSERTION RESULTS" section for more details')
