        ...
        bq_profile[profile_id] = {
            ...
            "outputs": {
                target: {
                    ...
                    "schema": '{{ env_var("DOP_DBT_SCHEMA", "'
                    + str(profile_id).replace("-", "_")
                    + '") }}',
                    "threads": 1,
                    "timeout_seconds": 300,
...
dop_config_path = os.path.sep.join(
    [build_dir, "embedded_dop", "executor_config", "dbt", "config.yaml"]
)
...
with open(dop_config_path) as fp_config:
    ...
    for dbt_config in dbt_configs:
        ...
        project_yml_path = os.path.sep.join(
            [build_dir, project_path, "dbt_project.yml"]
        )
        ...
        copy_and_overwrite(
            from_path=os.path.sep.join([build_dir, project_path]), to_path=to_path
        )
    ...
    if profile_ids:
        file_content = build_profile_file_content(profile_ids=profile_ids)
